  ...
  return (
    ...
    <div className="mx-auto flex min-h-screen w-full max-w-md flex-col bg-background pb-20">
      <motion.header
        ...
        className="sticky top-0 z-10 border-b border-border bg-background/95 backdrop-blur-xl"
      >
        <div className="px-5 pb-2.5 pt-3">
          <p className="label-eyebrow">Inbox</p>
          <div className="mt-1 flex items-end justify-between gap-3">
            <div className="min-w-0">
              <h1 className="font-serif-display text-3xl leading-none text-foreground">
                แชท
              </h1>
              <p className="mt-1.5 line-clamp-1 text-xs text-muted-foreground">
                แชทการเงินจะแสดงก่อนแชททั่วไป
              </p>
            </div>
            {totalUnreadCount > 0 ? (
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "chats" | "friends")} className="px-5 pb-2">
          <TabsList className="grid h-9 w-full grid-cols-2 rounded-md bg-muted p-0.5">
            <TabsTrigger
              value="chats"
              className="h-8 rounded-[calc(var(--radius)-2px)] px-2 text-xs data-[state=active]:bg-background data-[state=active]:shadow-none"
            >
    ...
    <div className="flex flex-col">
      ...
      {filteredFriends.length === 0 && searchQuery ? (
        <div className="flex flex-col items-center justify-center px-4 py-12">
          <p className="text-sm text-muted-foreground">ไม่พบเพื่อนที่ค้นหา</p>
        </div>
      ) : (
        <div className="divide-y divide-border">
          {filteredFriends.map((friend, index) => (
            <motion.div
              ...
              className={`flex cursor-pointer items-center gap-3 px-4 py-2.5 transition-colors active:bg-muted/60 ${
            ...
            >
              <div className="relative shrink-0">
                ...
                {friend.unreadCount > 0 && (
                  <span className="absolute -right-0.5 -top-0.5 flex h-[17px] min-w-[17px] items-center justify-center rounded-full bg-destructive px-1 text-[10px] font-medium text-destructive-foreground">
