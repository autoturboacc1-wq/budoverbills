...
import { QrCode, UserPlus, Search, Copy, Check, Scan, Loader2, Send } from "lucide-react";
...
export function AddFriendSection() {
  ...
  const handleSearchFriendByCode = () => {
    if (!requireAuth("search friend")) {
      navigate("/auth", { state: { from: { pathname: "/profile" } } });
      return;
    }
      ...
      >
        <div className="rounded-2xl border border-primary/20 bg-card p-4 shadow-sm">
          <div className="mb-4 flex items-center gap-2">
            ...
            <span className="font-medium text-foreground">เพิ่มเพื่อน</span>
          </div>
          <div id="add-friend-panel">
            {user && (
              <div className="mb-4 rounded-xl border border-border/80 bg-background p-3">
                <div className="mb-2 flex items-center justify-between gap-3">
                  <p className="text-sm font-medium text-foreground">รหัสของฉัน</p>
                  <Button
                    type="button"
                    variant="ghost"
                    size="sm"
                ...
                <div className="flex items-center gap-2">
                  <div className="min-w-0 flex-1 rounded-lg bg-secondary/50 px-3 py-2">
                    <p className="truncate font-mono text-lg font-bold tracking-widest text-foreground">
                      {userCode}
                    </p>
                  </div>
                  <Button
                  ...
                  >
                    {copied ? (
                      <Check className="h-4 w-4 text-status-paid" aria-hidden="true" />
                    ) : (
            ...
            <div className="space-y-2">
              <Button
                type="button"
                onClick={handleSearchFriendByCode}
                className="h-12 w-full justify-start gap-3"
              >
                <Search className="h-4 w-4" aria-hidden="true" />
                เพิ่มเพื่อนด้วยรหัสผู้ใช้
              </Button>
              <div className="grid grid-cols-2 gap-2">
                <Button
                  ...
                  variant="outline"
                  className="h-11 gap-2 border-primary/30 text-primary hover:bg-primary/10"
                >
