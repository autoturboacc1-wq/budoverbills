-- Fix Thai national ID checksum weights in sign_agreement_contract.
--
-- The previous version (20260425260000_id_card_full_13_digits.sql) iterated
-- v_i in 1..12 with weight (13 - v_i), producing weights 12,11,...,1.  The
-- correct Thai national ID Luhn-style algorithm uses weights 13,12,...,2 for
-- digits 1..12 (matching the JS validator in src/components/contract/PartyInfoForm.tsx).
-- The off-by-one rejected every otherwise-valid ID with
-- "Party idCardNumber checksum is invalid" once the user reached the RPC.
...
BEGIN
  ...
  -- Thai national ID Luhn-style checksum: digits 1..12 use weights 13..2,
  -- i.e. weight = 14 - position (since substring is 1-indexed in SQL).
  v_sum := 0;
  FOR v_i IN 1..12 LOOP
    v_sum := v_sum + CAST(substring(v_party_id FROM v_i FOR 1) AS int) * (14 - v_i);
  END LOOP;
