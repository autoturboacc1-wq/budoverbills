-- Hotfix: relax the agreement status check in submit_installment_slip.
--
-- The previous version (20260425220000) rejected anything that wasn't
-- 'active', which broke legitimate payment slips for agreements in
-- 'rescheduling' state.  The original confirm_installment_payment never
-- checked status at all (it only validated the lender role).  We keep a
-- check, but allow both 'active' and 'rescheduling' — the only states where
-- payments are still meaningful.
...
BEGIN
  ...
  -- Reject only the terminal / pre-active states.  'active' and
  -- 'rescheduling' are both valid for accepting payments.
  ...
  -- Direct insert (cross-user notify; SECURITY DEFINER bypasses RLS).
