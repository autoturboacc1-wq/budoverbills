-- BUG-DEBT-02: block duplicate pending reschedule requests for the same installment.
-- BUG-DEBT-03: verify installment is still pending before approving a reschedule request.
...
BEGIN
  ...
  -- BUG-DEBT-02: Block duplicate pending requests for the same installment
  IF EXISTS (
    SELECT 1 FROM reschedule_requests
    WHERE installment_id = p_installment_id
    AND status = 'pending'
  ) THEN
    RAISE EXCEPTION 'A pending reschedule request already exists for this installment';
  END IF;
...
BEGIN
  ...
  v_installment_id := v_request.installment_id;
  ...
  -- BUG-DEBT-03: Verify installment is still pending
  IF NOT EXISTS (
    SELECT 1 FROM installments
    WHERE id = v_installment_id
    AND status = 'pending'
  ) THEN
    RAISE EXCEPTION 'Installment is no longer pending';
  END IF;
  ...
  LOOP
    ...
    IF v_installment.id = v_installment_id THEN
