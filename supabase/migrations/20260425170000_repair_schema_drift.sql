-- Repair migration: re-add messages.voice_url and messages.voice_duration if missing.
--
-- Background: migration 20260402120000_add_voice_to_messages.sql is recorded as
-- applied in supabase_migrations.schema_migrations on production, but the
-- columns are not actually present (production runtime errors:
--   "column messages.voice_url does not exist" — code 42703).
-- This typically happens when the DB was reset/restored and the migrations
-- tracker was kept partial, so subsequent `db push` runs skip the original
-- ALTER TABLE.
--
-- The body below is identical to 20260402120000 and is idempotent
-- (`ADD COLUMN IF NOT EXISTS`), so re-running it is safe regardless of state.
...
-- Repair the financial-terms lock trigger.
-- Migration 20260407071000_fix_financial_terms_lock.sql references
-- `monthly_payment` on debt_agreements, but no migration ever creates that
-- column.  The reference is latent (short-circuited away when status is not
-- 'active') but throws "column monthly_payment does not exist" the moment a
-- lender legitimately edits an active agreement (e.g. bank info update).
-- Drop the bogus column ref; the other three columns already cover the
-- financial-terms invariant.
...
BEGIN
  ...
  -- Lock core financial terms once the agreement is active.
