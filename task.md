...
- [x] **BUG-ADMIN-03** — migration `20260113101429` — hardcoded `Admin@Secure2024!` ใน git history → rotate ทันที + ลบออกจาก migration

### 🗄️ RLS / Database
- [x] **BUG-RLS-02** — migration `20260131` — `tips` SELECT policy มี `OR auth.uid() IS NOT NULL` → ทุกคนอ่านข้อมูล tip ทุกคนได้ (deanonymize donors)
- [x] **BUG-RLS-03** — migration `20251231` — `installments` UPDATE policy ไม่มี column restriction → borrower self-confirm payment ได้โดยไม่ต้องส่งหลักฐาน
- [x] **BUG-RLS-05** — migration `20260109` — `activity_logs` SECURITY DEFINER `log_activity` ไม่เช็ค `auth.uid() = p_user_id` → false-flag attack (แจ้ง admin ว่าคนอื่น suspicious)
- [x] **BUG-RLS-07** — migration `20260125` — `user_points` INSERT policy → ตั้ง `total_points = 999999` ได้โดยตรง
- [x] **BUG-RLS-08** — migration `20260125` — `subscriptions` INSERT policy → insert `tier='premium', expires_at='2099'` ก่อน trigger ยิง = billing bypass
- [x] **BUG-RLS-12** — trigger `enforce_debt_agreement_role_updates` — `status` field ไม่ได้รับการป้องกัน → borrower `SET status='completed'` หนี้หายจาก dashboard
- [x] **BUG-RLS-01** — `create_notification` SECURITY DEFINER — ไม่เช็ค caller = target → inject notification ปลอมให้ใครก็ได้
...
### 🗄️ RLS / Database
- [x] **BUG-RLS-09** — `admin_otp` table — missing columns `failed_attempts` / `locked_until` → OTP lockout พัง = brute-force ไม่จำกัด
- [x] **BUG-RLS-11** — `chat_rooms` INSERT policy → attacker สร้าง fake debt room ใส่ inbox ใครก็ได้
- [x] **BUG-RLS-21** — `create_agreement_with_installments` RPC — ข้าม quota check → สร้าง agreement ไม่จำกัดโดยเรียก RPC โดยตรง
- [x] **BUG-RLS-22** — trigger `enforce_debt_agreement_role_updates` — ไม่ lock financial terms หลัง active → lender เปลี่ยน `interest_rate=99%` หลัง borrower confirm
- [x] **BUG-RLS-16** — `add_agreement_credits` — user-path ยังอยู่ → self-grant credits
- [x] **BUG-RLS-06** — tips INSERT RPC `record_tip` ไม่เช็ค `auth.uid() = p_user_id` → แต่งข้อมูล tip ของคนอื่น

### 💬 Chat
- [x] **BUG-CHAT-01** — `src/pages/Chat.tsx:67-89` — Realtime subscription ไม่มี filter → subscribe all messages ทุก user
- [x] **BUG-RLS-02 (messages)** — `messages` UPDATE policy ไม่มี `WITH CHECK (sender_id)` → participant แก้ `image_url` message ของอีกฝ่ายได้ (forged payment slip)
- [x] **BUG-CHAT-02** — `src/components/chat/ChatRoom.tsx:234-256` — `console.log` ใน production leak UUIDs + message content

### 📱 Notifications / PWA
- [x] **BUG-NOTIF-03** — `dist/sw.js` — ไม่มี `push` event handler → push notifications ไม่ทำงานเลย

### 💳 Payment
- [x] **BUG-PAY-01** — `src/components/PaymentDialog.tsx:198-247` — race condition: storage upload + RPC แยก operation → orphaned slip files
...
- [x] **BUG-MONEY-07 / BUG-PDF-07** — `src/utils/pdfExport.ts:95-99` — Helvetica font ไม่มี Thai glyphs → ชื่อ/ข้อความภาษาไทยใน PDF เป็น □□□□
...
- [x] **BUG-AUTH-07** — `src/contexts/AuthContext.tsx:58-82` — race condition ระหว่าง `authLoading` กับ `profileLoading`

### 📝 Agreement
- [x] **BUG-AGREE-01** — `src/pages/CreateAgreement.tsx:218-245` — non-atomic quota check + creation (TOCTOU race)
...
### 💳 Payment
- [x] **BUG-PAY-03** — migration `20260404143000` — `process_extra_payment` principal rounding drift → 1-cent gap ค้างอยู่ตลอด
...
- [x] **BUG-DEBT-02** — migration `20260406090000` — `submit_reschedule_request` ไม่ block duplicate pending requests
- [x] **BUG-DEBT-03** — migration `20260406090000` — `approve_reschedule_request` ไม่เช็คว่า installment ยังเป็น `pending`

### 💬 Chat
- [x] **BUG-CHAT-03** — `src/components/chat/ChatRoom.tsx` — ไม่มี message pagination → load ทั้งหมดทุกครั้ง
- [x] **BUG-CHAT-04** — `src/components/chat/VoiceRecorder.tsx:145-162` — stale closure ใน auto-stop timer → recording ที่ 2 ถูกหยุดผิดเวลา
- [x] **BUG-CHAT-05** — migration `20260404210000` — partial UNIQUE INDEX ใช้เป็น PostgREST conflict target ไม่ได้ → typing status stuck

### 👥 Friends
- [x] **BUG-FRIEND-01** — `src/hooks/useFriendRequests.ts:145-168` — ไม่ award points เมื่อ accept friend request
- [x] **BUG-FRIEND-02** — `src/components/AddFriendSection.tsx:78-95` — QR path ใช้ direct query แทน `search_profile_by_code` → bypass rate limit + leak full profile
- [x] **BUG-FRIEND-03** — `friend_requests` UPDATE RLS ไม่มี `WITH CHECK` → recipient แก้ `from_user_id` ได้
...
### 👤 Profile
- [x] **BUG-PROF-01** — `src/components/AvatarUpload.tsx:89-145` — old avatar ไม่ลบ + MIME จาก browser เท่านั้น
- [x] **BUG-PROF-02** — `src/lib/validation.ts:156-170` — validate bank account 10 digits เท่านั้น → GSB/BAAC (12 digits) ใช้ไม่ได้

### 🛠️ Admin
- [x] **BUG-ADMIN-04** — `src/utils/adminSession.ts:45-67` — `setAdminSession()` ไม่ set `admin_code_verified` flag
- [x] **BUG-ADMIN-05** — `supabase/functions/admin-session/index.ts` — `Access-Control-Allow-Origin: *` บน admin endpoint
- [x] **BUG-ADMIN-06** — `src/pages/admin/AdminUsersPage.tsx:178-210` — user search client-side บน LIMIT 100 → ไม่เจอ user ที่เกิน 100 คนแรก

### 🏠 Dashboard
- [x] **BUG-DASH-01** — `src/components/PaymentCalendar.tsx:89-134` — ไม่ใช้ Bangkok timezone → due dates ผิดวันสำหรับ user ที่ท่องเที่ยว
...
- [x] **BUG-HOOK-05** — `src/hooks/useUserPoints.ts:155-189` — first-time user creation race condition
...
- [x] **BUG-RT-03** — `src/App.tsx:45-78` — providers remount ทุก navigation → kills Realtime subscriptions
...
- [x] **BUG-SEC-07** — `src/hooks/useRateLimiter.ts` — rate limiter client-side เท่านั้น → bypass ด้วย F5 หรือ fetch โดยตรง
- [x] **BUG-SEC-09** — `src/contexts/AuthContext.tsx:43-48` — `getSafeAuthDestination` ไม่ decode URL ก่อน validate → `/\evil.com` ผ่านได้

### 🏅 Points/History
- [x] **BUG-POINTS-03** — `src/hooks/useUserPoints.ts:145-178` — `redeemPoints` double-spend: 2 tabs อ่าน balance เดิม → spend 2x
- [x] **BUG-POINTS-04** — `src/hooks/useUserPoints.ts:178-198` — daily reset client-side → reset ไม่ fire ถ้าเปิด app ค้างคืน

### 🌐 RLS (Medium)
- [x] **BUG-RLS-13** — `can_create_agreement_free` — ไม่เช็ค caller = p_user_id → probe quota ของคนอื่นได้
- [x] **BUG-RLS-19** — `messages` UPDATE trigger — `image_url`, `file_url` columns ไม่ได้รับการป้องกัน
- [x] **BUG-RLS-24** — `profiles` SELECT policy — `auth.uid() IS NOT NULL` → อ่าน phone/name ของทุกคนได้ (PDPA violation)
...
### 📦 i18n / PWA / Build
- [x] **BUG-I18N-01** — `src/components/PWAInstallPrompt.tsx` — hardcoded Thai strings ไม่ใช้ `t()`
- [x] **BUG-I18N-02** — `vite.config.ts` — `sourcemap: false` → production errors debug ไม่ได้ใน Sentry
- [x] **BUG-I18N-03** — `src/pages/NotFound.tsx` — ใช้ `<a href="/">` แทน `<Link>` → full page reload + English only
- [x] **BUG-I18N-04** — `src/components/ux/PageTransition.tsx` — `minHeight: 100dvh` → layout shift ระหว่าง AnimatePresence
- [x] **BUG-I18N-05** — `src/App.tsx` — `<Suspense>` inside `<AnimatePresence>` → double animation ทุก lazy route

### 🧭 Routing / UX
- [x] **BUG-UX-01** — `src/components/BottomNav.tsx:215` — active state ใช้ strict equality → ไม่ highlight บน nested routes
- [x] **BUG-UX-02** — `src/App.tsx:45-78` — providers remount ทุก redirect → state/subscription loss
- [x] **BUG-UX-04** — `src/hooks/useDebtAgreements.ts` — ไม่มี skeleton loader ระหว่าง fetch

### 🔒 Security (Low)
- [x] **BUG-SEC-08** — auth JWT ใน `localStorage` → XSS exfiltration target (trade-off, แก้ด้วย CSP)
- [x] **BUG-SEC-10** — `src/utils/adminSession.ts` — ไม่ store `expires_at` client-side → admin UI ยังแสดงหลัง token หมดอายุ
- [x] **BUG-SEC-11** — `src/components/ui/chart.tsx:70-83` — CSS injection ผ่าน `dangerouslySetInnerHTML` (latent)
- [x] **BUG-SEC-12** — `src/utils/promptpay.ts` — TLV `padStart(2)` ไม่ cap → 3-digit length field ถ้า value > 99 chars (future risk)

### 🗄️ RLS (Low)
- [x] **BUG-RLS-20** — avatars CDN URL accessible ตลอดแม้ delete account
- [x] **BUG-RLS-23** — `reschedule_requests` policy ไม่ใช้ `public.` prefix
- [x] **BUG-RLS-25** — `chat_rooms` UPDATE → participant reset pending_action_type เองได้
- [x] **BUG-RLS-26** — `verify_admin_otp_and_issue_session` ไม่เช็ค `auth.uid() = p_user_id`
- [x] **BUG-RLS-27** — `generate_admin_otp` — ใครก็ invalidate admin OTP คนอื่นได้
- [x] **BUG-RLS-28** — `chat-attachments` bucket ไม่มี MIME type หรือ size limit
- [x] **BUG-RLS-29** — lock ordering inconsistency ใน payment RPCs → deadlock risk

### 📊 Domain / Money (Low)
- [x] **BUG-DOMAIN-04** — `src/hooks/useDebtAgreements.ts:150-156` — `as unknown as` double-cast ลบ type safety
...
- [x] **BUG-PAY-05** — `src/components/PaymentDialog.tsx:285-310` — blob URL ไม่ revoke → memory leak
- [x] **BUG-PROF-04** — `src/components/AvatarUpload.tsx:112` — file size validation client-side เท่านั้น
- [x] **BUG-FRIEND-04** — `src/hooks/useFriendRequests.ts:62-80` — duplicate check in-memory เท่านั้น
...
|----------|-------|------|-----------|
| P0 Critical | 14 | 14 | 0 |
| P1 High Security | 25 | 25 | 0 |
| P2 High/Medium | 54 | 54 | 0 |
| P3 Low | 25 | 25 | 0 |
| **Total** | **118** | **118** | **0** |
